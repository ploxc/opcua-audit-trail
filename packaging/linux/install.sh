...
if [ ! -f "$ETC/config.toml" ]; then
    "$BIN" --config "$ETC/config.toml" init >/dev/null
    # Keep certificates and data out of /etc.
    sed -i "s|^pki_dir = .*|pki_dir = \"$DATA/pki\"|; s|^data_dir = .*|data_dir = \"$DATA\"|" "$ETC/config.toml"
    rm -rf "$ETC/pki"
    chown -R opcua-gw:opcua-gw "$ETC"
    runuser -u opcua-gw -- "$BIN" --config "$ETC/config.toml" init >/dev/null
    echo "created $ETC/config.toml: add your targets, then restart the service"
fi
chown -R opcua-gw:opcua-gw "$DATA"
...
echo "admin password (first start only): journalctl -u opcua-audit-gateway | grep password"
