...
A node can be in only one group per client, and not in a client's group
when a group for every client has it already. The web UI greys out such
choices.
...
Older (`[[targets.ignore]]`) or overlapping files still load and are
corrected.
